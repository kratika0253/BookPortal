...
int traversingrows(int tar[10], int bar[10], int iar[10], int isize, int jsize) 
{
	int t, b, i, qi,ri, z = 0;
	for (i = 0; i <= isize; i++) 
	{
...
int traversingcoloumns(int lar[10], int rar[10], int jar[10], int isize, int jsize) 
{
	int l, r, j, qj, rj, z = 0;
	for (j = 0; j <= jsize; j++) 
	{
...
void printMatrix(int a[10][10], int isize, int jsize) 
{
...
{ 
	...
	int dyn,jsize,isize, j, i, m, n, p, k, c = 0, itrcount=0;
	...
	int lar[10], rar[10], jar[10], tar[10], bar[10], iar[10];
